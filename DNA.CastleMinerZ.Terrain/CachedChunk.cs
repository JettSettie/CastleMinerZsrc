...
namespace DNA.CastleMinerZ.Terrain
{
	public class CachedChunk : IReleaseable, ILinkedListNode
	{
		...
		public static IntVector3 MakeChunkCornerFromCID(int cid)
		{
			...
			short num = (short)((int)(cid & 4294901760u) >> 16);
		...
		{
			int num = 0;
			uint num2 = (uint)((chunkCorner.Z / 16) & 0xFFFF);
			uint num3 = (uint)((chunkCorner.X / 16) & 0xFFFF);
					...
					CastleMinerZGame.Instance.SaveDevice.Load(fileName, delegate(Stream stream)
					{
						...
						else
						{
							...
							for (int k = 0; k < num; k++)
							{
								...
								uint num6 = (uint)((int)num5 & -16777216) >> 24;
		...
		{
			if (_loadingPriority < priority)
			{
				_loadingPriority = priority;
			}
			ChunkCache.Instance.GetChunkFromServer(_worldMin, _loadingPriority);
		...
		public void MaybeRetryFetch()
		{
			...
			double num2 = (_loadingPriority <= 0) ? 20.0 : ((_retries >= 2) ? 0.5 : 2.0);
			if (num > num2)
			{
				_retries++;
				GetChunkFromHost(_loadingPriority);

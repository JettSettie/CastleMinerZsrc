...
namespace DNA.CastleMinerZ.Terrain
{
	public class BlockTerrain : Entity
	{
		...
		public struct TerrainChunk
		{
			...
			{
				PendingMod pendingMod = _mods.Clear();
				while (pendingMod != null)
				{
					PendingMod pendingMod2 = pendingMod.NextNode as PendingMod;
					pendingMod.NextNode = null;
					pendingMod.Release();
					pendingMod = pendingMod2;
				}
			}

			public void SwapIn(ref TerrainChunk newChunk)
			{
				if (_chunk != null)
				{
					ZeroData(true);
		...
		private static readonly int[] _faceIndexNeighbors = new int[6]
		{
			128,
			-49152,
			-128,
		...
		private List<QueuedBufferBuild> _vertexBuildListIncoming = new List<QueuedBufferBuild>();

		private List<QueuedBufferBuild> _vertexBuildListOutgoing = new List<QueuedBufferBuild>();
		...
		public float DrawDistance
		{
			...
			set
			{
				...
				if (value != _drawDistance)
				{
					int num2 = (int)Math.Floor(8f * num) + 4;
					_farthestDrawDistanceSQ = num2 * num2;
					_drawDistance = value;
				}
			}
		}
		...
		public BlockTerrain(GraphicsDevice gd, ContentManager cm)
		{
			...
			_graphicsDevice = gd;
			IsWaterWorld = false;
			_effect = cm.Load<Effect>("Shaders\\BlockEffect");
			Texture[] array = null;
			...
			if (GraphicsProfileManager.Instance.IsHiDef)
			{
				...
				texture2D.GetData(data, 0, texture2D.Bounds.Width * texture2D.Bounds.Height);
				texture2D = cm.Load<Texture2D>("Textures\\EnvMaps\\envmap1");
				texture2D.GetData(data, texture2D.Bounds.Width * texture2D.Bounds.Height, texture2D.Bounds.Width * texture2D.Bounds.Height);
				texture2D = cm.Load<Texture2D>("Textures\\EnvMaps\\envmap2");
				texture2D.GetData(data, texture2D.Bounds.Width * texture2D.Bounds.Height * 2, texture2D.Bounds.Width * texture2D.Bounds.Height);
				texture2D = cm.Load<Texture2D>("Textures\\EnvMaps\\envmap3");
				texture2D.GetData(data, texture2D.Bounds.Width * texture2D.Bounds.Height * 3, texture2D.Bounds.Width * texture2D.Bounds.Height);
				_envMap = new Texture3D(gd, texture2D.Bounds.Width, texture2D.Bounds.Height, 4, false, texture2D.Format);
				_envMap.SetData(data);
				texture2D = null;
				data = null;
			}
			else
			...
			Vector2 vector = new Vector2(0.5f / (float)_diffuseAlpha.Width, 0.5f / (float)_diffuseAlpha.Height);
			...
			_updateLightingPool = new LightingPool(7500);
		...
		public void DoThreadedCleanup(object context)
		{
			...
			{
				if (stopwatch.ElapsedMilliseconds > 120000 && TaskDispatcher.Instance.IsIdle(TaskThreadEnum.NUM_THREADS))
				{
		...
		public void Reset()
		{
			...
			if (IsReady)
			{
				...
				ThreadPool.QueueUserWorkItem(DoThreadedReset);
			}
		}
		...
		public void Teleport(Vector3 newPosition)
		{
			...
			if (IsReady)
			{
				...
				ThreadPool.QueueUserWorkItem(DoThreadedCleanup);
			}
		}
			...
			if (!_resetRequested && !buildTaskData._skipProcessing)
			{
				if (_chunks[buildTaskData._intData0]._action >= NextChunkAction.NEEDS_GEOMETRY)
				{
					...
					if (intVector.X >= 0 && intVector.X < 384 && intVector.Z >= 0 && intVector.Z < 384)
					{
						...
						if (renderChunk.BuildFaces(_graphicsDevice))
						{
							lock (_vertexBuildListLockObject)
							{
								_vertexBuildListIncoming.Add(new QueuedBufferBuild(buildTaskData._intData0, renderChunk));
							}
							flag = true;
						}
						else
			...
			if (_shiftTerrainData.running && _shiftTerrainData.done)
			{
				...
				if (dx > 0)
				{
					...
					for (int i = 0; i < 24; i++)
					{
						...
						while (num7 < dx)
						{
							if (_chunks[num6]._chunk != null)
							{
								_chunks[num6].ZeroData(true);
							}
							num7++;
							num6++;
						}
				...
				else if (dx < 0)
				{
					...
					for (int j = 0; j < 24; j++)
					{
						...
						while (num9 < -dx)
						{
							if (_chunks[num8]._chunk != null)
							{
								_chunks[num8].ZeroData(true);
							}
							num9++;
							num8--;
						}
				...
				{
					_chunks[num4 + num3].SwapIn(ref _chunks[num4]);
					num4 += num5;
				}
				for (int l = 0; l < 576; l++)
				{
					if (_chunks[l]._chunk == null)
					{
						_chunks[l].Reset();
					}
				}
		...
		private bool SlopingIsBlocked(ref IntVector3 min, ref IntVector3 max, ref IntVector3 test)
		{
			...
			while (a.Z <= max.Z)
			{
				...
				while (a.X <= max.X)
				{
					a.Y = test.Y + 1;
					int num = MakeIndex(a);
					if (!Block.GetType(_blocks[num]).BlockPlayer && Block.GetType(_blocks[num + 1]).BlockPlayer)
					{
						return true;
					}
					a.X++;
		...
		{
			IntVector3 min = MakeIndexVectorFromPosition(tp._bounds.Min);
			IntVector3 max = MakeIndexVectorFromPosition(tp._bounds.Max);
			bool flag = false;
			int num = 0;
			int num2 = 0;
			if (tp.SimulateSlopedSides)
			{
				...
				IntVector3 localIndex = GetLocalIndex(IntVector3.FromVector3(start));
				localIndex.Y++;
				if (IsIndexValid(localIndex))
				{
					int num3 = MakeIndex(localIndex);
					if (!Block.GetType(_blocks[num3]).BlockPlayer)
					{
						num = (int)Math.Floor(tp._start.Y - tp.HalfVector.Y + 0.01f) - _worldMin.Y;
						num2 = (int)Math.Floor(tp._start.Y - tp.HalfVector.Y + 0.2f) - _worldMin.Y;
						flag = true;
					}
				}
			}
			tp.ShapeHasSlopedSides = false;
			min.SetToMax(IntVector3.Zero);
			max.SetToMin(new IntVector3(383, 127, 383));
			IntVector3 intVector = IntVector3.Subtract(min, max);
			if (intVector.X * intVector.Y * intVector.Z >= 27)
			{
				return;
			}
			IntVector3 test = min;
			...
			while (test.Z <= max.Z)
			{
				...
				{
					test.Y = min.Y;
					int num4 = MakeIndex(test);
					while (test.Y <= max.Y)
					{
						if (tp.TestThisType(Block.GetTypeIndex(_blocks[num4])))
						{
							if (flag && Block.GetType(_blocks[num4]).AllowSlopes)
							{
								if (test.Y <= num2 && test.Y >= num)
								{
									tp.ShapeHasSlopedSides = !SlopingIsBlocked(ref min, ref max, ref test);
								}
								else
								{
									tp.ShapeHasSlopedSides = false;
							...
							IntVector3 intVector2 = test + _worldMin;
							FillFacePlanes(intVector2);
							tp.TestShape(_facePlanes, intVector2, Block.GetTypeIndex(_blocks[num4]));
						}
						test.Y++;
						num4++;
					}
		...
		public int DepthUnderSpaceRock(Vector3 location)
		{
			...
			for (int i = a.Y; i < 128; i++)
			{
				a.Y = i;
				int num2 = MakeIndex(a);
				if (Block.GetType(_blocks[num2])._type == BlockTypeEnum.SpaceRock)
				{
					num++;
				}
			}
		...
		public bool PointIsInAsteroid(Vector3 guess)
		{
			...
			IntVector3 a = MakeIndexVectorFromPosition(guess);
			if (!IsIndexValid(a))
			{
				return false;
			}
			int num = MakeIndex(a);
			if (Block.IsSky(_blocks[num]))
			{
				return false;
			}
			int num2 = num;
			...
			while (num3 < 128)
			{
				if (Block.GetTypeIndex(_blocks[num2]) == BlockTypeEnum.SpaceRock)
				{
					flag = true;
					break;
				}
		...
		private int FindStepableHeight(ref IntVector3 v, int currentHeight)
		{
			...
			int num = MakeIndex(v);
			if (Block.GetType(_blocks[num + currentHeight]).BlockPlayer)
			{
				int i = currentHeight;
				...
				for (; i < num2; i++)
				{
					if (!Block.GetType(_blocks[num + i]).BlockPlayer)
					{
						flag = true;
						break;
					}
				...
				int num3 = 0;
				while (num3 < 2 && i < 128 && !Block.GetType(_blocks[num + i]).BlockPlayer)
				{
					num3++;
					i++;
				}
			...
			int j = currentHeight;
			for (int num4 = Math.Min(currentHeight + 3, 128); j < num4 && !Block.GetType(_blocks[num + j]).BlockPlayer; j++)
			{
			}
			if (j - currentHeight < 2)
			{
				return -1;
			}
			j = currentHeight;
			while (j > 0 && !Block.GetType(_blocks[num + j]).BlockPlayer)
			{
				j--;
			}
			j++;
			if (Math.Abs(j - currentHeight) > 2)
			{
				return -1;
			}
			return j;
		...
		{
			plrpos.X = (float)Math.Floor(plrpos.X) + 0.1f;
			plrpos.Y = (float)Math.Floor(plrpos.Y) + 0.1f;
			plrpos.Z = (float)Math.Floor(plrpos.Z) + 0.1f;
			IntVector3 intVector = MakeIndexVectorFromPosition(plrpos);
			if (!IsIndexValid(intVector))
			{
				return Vector3.Zero;
			}
			...
			int num = intVector.Y;
			intVector2.Y = 0;
			int num2 = MakeIndex(intVector2);
			while (num > 0 && !Block.GetType(_blocks[num2 + num]).BlockPlayer)
			{
				num--;
			}
			if (num < 0)
			...
			for (num++; num < 128 && Block.GetType(_blocks[num2 + num]).BlockPlayer; num++)
			{
			}
			if (num >= 128)
			{
			...
			{
				bool flag = false;
				int num4 = (num3 != -1 || !MathTools.RandomBool()) ? (num3 ^ 2) : MathTools.RandomInt(0, 4);
				int num5 = 0;
				while (!flag)
				{
					if (num4 != num3)
					{
						IntVector3 v = intVector2;
						if ((num4 & 1) != 0)
						{
							v.X += (((num4 & 2) != 0) ? 1 : (-1));
						}
						...
						int num6 = FindStepableHeight(ref v, num);
						if (num6 != -1)
						{
							intVector2 = v;
							num = num6;
			...
			intVector2.Y = num;
			intVector2 = IntVector3.Add(_worldMin, intVector2);
			Vector3 result = IntVector3.ToVector3(intVector2);
			result.X += 0.5f;
			result.Y += 0.1f;
			result.Z += 0.5f;
			return result;
		...
		public Vector3 FindAlienSpawnPoint(Vector3 guess, bool surfaceOkay)
		{
			...
			{
				BlockTypeEnum typeIndex = Block.GetTypeIndex(_blocks[num + intVector.Y]);
				if (!surfaceOkay && !flag)
				{
				...
				{
					num5 = ((!BlockType.GetType(typeIndex).BlockPlayer) ? (num5 + 1) : 0);
				}
				intVector.Y--;
			}
			if (num2 != -1)
			{
				intVector.Y = num2 + 1;
				Vector3 result = MakePositionFromIndexVector(intVector);
				result.X += 0.5f;
				result.Y += 0.1f;
				result.Z += 0.5f;
				return result;
		...
		{
			IntVector3 intVector = MakeIndexVectorFromPosition(center);
			...
			for (int i = 0; i < radius * 2; i++)
			{
				...
				for (int j = 0; j < radius * 2; j++)
				{
					...
					float num = MathTools.Square(a.X - intVector.X) + MathTools.Square(a.Z - intVector.Z);
					...
					while (num3 < 128)
					{
						if (Block.GetTypeIndex(_blocks[num2]) == type)
						{
							distToClosest = num;
							break;
						}
			...
			{
				distToClosest = (float)Math.Sqrt(distToClosest);
				return true;
			}
			return false;
		}
		...
		public void StepInitialization()
		{
			...
			while (true)
			{
				if (num3 >= _radiusOrderOffsets.Length)
				{
					return;
				}
				IntVector3 intVector2 = IntVector3.Add(a, _radiusOrderOffsets[num3]);
				if (intVector2.X >= 0 && intVector2.X < 24 && intVector2.Z >= 0 && intVector2.Z < 24)
				{
					num4 = intVector2.X + intVector2.Z * 24;
					if (_chunks[num4]._action == NextChunkAction.NONE && !_chunks[num4]._mods.Empty && !ChunkOrNeighborInUse(num4) && ApplyModList(num4))
					{
						break;
					}
				}
		...
		public Vector2 GetLightAtPoint(Vector3 position)
		{
			...
			Vector3 value = IntVector3.ToVector3(intVector);
			...
			while (zero.Z < 1.5f)
			{
				...
				while (zero.Y < 1.5f)
				{
					...
					while (zero.X < 1.5f)
					{
						float num6 = ((2.25f - (value + zero - position).LengthSquared()) / 2.25f).Clamp(0f, 1f);
		...
		{
			Vector3 value = BelowWaterColor.ToVector3();
			return value * SunlightColor.ToVector3();
		}
		...
		{
			lock (_cleanupVertexBuildListLockObject)
			{
				lock (_vertexBuildListLockObject)
				{
					for (int i = 0; i < 2; i++)
					{
						List<QueuedBufferBuild> list = (i == 0) ? _vertexBuildListIncoming : _vertexBuildListOutgoing;
						for (int j = 0; j < list.Count; j++)
						{
							QueuedBufferBuild queuedBufferBuild = list[j];
							queuedBufferBuild.Chunk.SkipBuildingBuffers();
							queuedBufferBuild.Chunk.Release();
							_chunks[queuedBufferBuild.Index]._action = NextChunkAction.NONE;
							_chunks[queuedBufferBuild.Index]._numUsers.Decrement();
						}
						list.Clear();
					}
				}
		...
		public void BuildPendingVertexBuffers()
		{
			lock (_cleanupVertexBuildListLockObject)
			{
				lock (_vertexBuildListLockObject)
				{
					List<QueuedBufferBuild> vertexBuildListIncoming = _vertexBuildListIncoming;
					_vertexBuildListIncoming = _vertexBuildListOutgoing;
					_vertexBuildListOutgoing = vertexBuildListIncoming;
				}
				for (int i = 0; i < _vertexBuildListOutgoing.Count; i++)
				{
					QueuedBufferBuild queuedBufferBuild = _vertexBuildListOutgoing[i];
					queuedBufferBuild.Chunk.FinishBuildingBuffers(_graphicsDevice);
					_chunks[queuedBufferBuild.Index].ReplaceChunk(queuedBufferBuild.Chunk);
					queuedBufferBuild.Chunk.Release();
				...
				_vertexBuildListOutgoing.Clear();
			}
		}
			...
			else
			{
				using (Profiler.TimeSection("Drawing Terrain", ProfilerThreadEnum.MAIN))
				{
					if (IsReady)
					{
						...
						{
							float amount = (float)(int)AmbientSunColor.G / 255f;
							Vector3 value2 = Vector3.Lerp(new Vector3(0.9f, 0.95f, 1f), SunlightColor.ToVector3(), amount);
							_effect.Parameters["SunLight"].SetValue(value2);
							value2 = Vector3.Lerp(new Vector3(0.9f, 0.95f, 1f), AmbientSunColor.ToVector3(), amount);
							_effect.Parameters["AmbientSun"].SetValue(value2);
						}
						else
						{
							_effect.Parameters["SunLight"].SetValue(SunlightColor.ToVector3());
						...
						for (int i = 0; i < _radiusOrderOffsets.Length; i++)
						{
							IntVector3 b = _radiusOrderOffsets[i];
							if (b.X * b.X < _farthestDrawDistanceSQ && b.Y * b.Y < _farthestDrawDistanceSQ && b.Z * b.Z < _farthestDrawDistanceSQ)
							{
								IntVector3 intVector = IntVector3.Add(chunkVectorIndex, b);
								if (intVector.X >= 0 && intVector.X < 24 && intVector.Z >= 0 && intVector.Z < 24)
								{
									int num2 = intVector.X + intVector.Z * 24;
									RenderChunk chunk = _chunks[num2].GetChunk();
						...
						if (num > 0)
						{
							...
							{
								_effect.CurrentTechnique = _effect.Techniques[UseSimpleShader ? 3 : 2];
							}
							device.BlendState = BlendState.Opaque;
							for (int j = 0; j < 2; j++)
							{
								for (int k = 0; k < num; k++)
								{
									...
									chunk2.Draw(_graphicsDevice, _effect, j == 1, _boundingFrustum);
									chunk2.Release();
								}
							}
							device.BlendState = BlendState.NonPremultiplied;

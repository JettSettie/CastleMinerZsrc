...
namespace DNA.CastleMinerZ.Terrain
{
	public class RenderChunk : IReleaseable, ILinkedListNode
	{
		...
		private static readonly float[] faceBrightness = new float[10]
		{
			0.92f,
			0.8367f,
			0.92f,
		...
		public bool BuildFaces(GraphicsDevice gd)
		{
			...
			if (blockBuildData.HasVertexes)
			{
				if (_vbs == null)
				{
					_vbs = new List<VertexBufferKeeper>();
				}
				if (_fancyVBs == null)
				{
					_fancyVBs = new List<VertexBufferKeeper>();
				}
				...
				_pendingBuildData = blockBuildData;
				return true;
			}
			blockBuildData.Release();
			return false;
		...
		{
			_pendingBuildData.Release();
			_pendingBuildData = null;
		}

		public void FinishBuildingBuffers(GraphicsDevice gd)
		{
			_pendingBuildData.BuildVBs(gd, ref _vbs, ref _fancyVBs);
			_pendingBuildData.Release();
			_pendingBuildData = null;
		}
		...
		public void DrawReflection(GraphicsDevice gd, Effect effect, BoundingFrustum frustum)
		{
			...
			if (_vbs != null && _vbs.Count > 0)
			{
				...
				{
					VertexBuffer buffer = _vbs[i].Buffer;
					int numVertexesUsed = _vbs[i].NumVertexesUsed;
			...
			if (_fancyVBs != null && _fancyVBs.Count > 0)
			{
				...
				{
					VertexBuffer buffer2 = _fancyVBs[j].Buffer;
					int numVertexesUsed2 = _fancyVBs[j].NumVertexesUsed;
		...
		public void Draw(GraphicsDevice gd, Effect effect, bool fancy, BoundingFrustum frustum)
		{
			...
			if (fancy)
			{
				if (_fancyVBs != null && _fancyVBs.Count > 0)
				{
					...
					{
						VertexBuffer buffer = _fancyVBs[i].Buffer;
						int numVertexesUsed = _fancyVBs[i].NumVertexesUsed;
						gd.SetVertexBuffer(buffer);
			...
			else if (_vbs != null && _vbs.Count > 0)
			{
				...
				{
					VertexBuffer buffer2 = _vbs[j].Buffer;
					int numVertexesUsed2 = _vbs[j].NumVertexesUsed;
					gd.SetVertexBuffer(buffer2);

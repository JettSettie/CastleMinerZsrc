...
namespace DNA.CastleMinerZ.Utils.Trace
{
	public class AABBTraceProbe : TraceProbe
	{
		...
		public override bool TestShape(Plane[] planes, IntVector3 worldIndex)
		{
			...
			if (num <= num2 && (!hasDirection || Math.Abs(Vector3.Dot(_direction, planes[num3].Normal)) > 1E-05f || (flag && (SimulateSlopedSides || !SkipEmbedded))))
			{
				SetIntersection(num, ref planes[num3].Normal, flag, (BlockFace)num3, num2, ref planes[num4].Normal, endsIn, (BlockFace)num4, worldIndex);
			}
			return true;
		}
	}

...
namespace DNA.CastleMinerZ
{
	public class BlockEntity : Entity
	{
		...
		public static void Initialize()
		{
			...
			_vb = new VertexBuffer(instance.GraphicsDevice, typeof(BlockEntityVertex), 24, BufferUsage.WriteOnly);
		...
		protected override void OnUpdate(GameTime gameTime)
		{
			if (!_uiObject && BlockTerrain.Instance != null && BlockTerrain.Instance.IsReady)
			{
				...
				TorchLight = Math.Max(125f, lightAtPoint.Y * 255f);
			}
		...
		public override void Draw(GraphicsDevice device, GameTime gameTime, Matrix view, Matrix projection)
		{
			...
			if (_uiObject)
			{
				...
				_effect.CurrentTechnique = _effect.Techniques[6];
				_effect.CurrentTechnique.Passes[type.NeedsFancyLighting ? 3 : 2].Apply();
			}
			else if (CastleMinerZGame.Instance.DrawingReflection)
			{
				_effect.CurrentTechnique = _effect.Techniques[4];
				_effect.CurrentTechnique.Passes[2].Apply();
			}
			else
			{
				_effect.CurrentTechnique = _effect.Techniques[BlockTerrain.Instance.IsWaterWorld ? 4 : 6];

...
namespace DNA.CastleMinerZ
{
	public class TracerManager : Entity
	{
		...
		public class Tracer
		{
			...
			{
				Vector3 value = (!(vel.LengthSquared() < 0.001f)) ? Vector3.Up : Vector3.Normalize(vel);
				Head = (Tail = pos + value * 0.5f);
			...
			public void Update(float dt)
			{
				...
				else
				{
					if (CastleMinerZGame.Instance.CurrentNetworkSession != null && CastleMinerZGame.Instance.PVPState != 0)
					{
						...
						for (int i = 0; i < CastleMinerZGame.Instance.CurrentNetworkSession.AllGamers.Count; i++)
						{
							...
							if (player2.ValidLivingGamer)
							{
								...
								tp.Reset();
								tp.TestBoundBox(playerAABB2);
								if (tp._collides && tp._inT < num3)
								{
									player = player2;
						...
						if (flag4)
						{
							...
							if (flag3)
							{
								if (player.IsLocal && ShooterID != player.Gamer.Id)
								{
									...
									{
										InGameHUD.Instance.ApplyDamage(0.21f, Head);
									}
								}
					...
					if (tp._collides)
					{
						...
						BlockTypeEnum blockWithChanges = BlockTerrain.Instance.GetBlockWithChanges(tp._worldIndex);
						if (blockWithChanges == BlockTypeEnum.TNT || blockWithChanges == BlockTypeEnum.C4)
						{
							DetonateExplosiveMessage.Send(CastleMinerZGame.Instance.MyNetworkGamer, tp._worldIndex, true, (blockWithChanges != BlockTypeEnum.TNT) ? ExplosiveTypes.C4 : ExplosiveTypes.TNT);
						}
						if (shootableEnemy == null)
						{
			...
			public int AddToDrawCache(TracerVertex[] vertexCache, int index)
			{
				...
				Vector3 value = Vector3.Lerp(Head, Tail, 0.1f);
				vertexCache[num].Position = value + vector2 * 0.025f;
				vertexCache[num++].Color = TracerColor;
				vertexCache[num].Position = value - vector2 * 0.025f * 0.5f - vector3 * 0.025f * 0.866f;
				vertexCache[num++].Color = TracerColor;
				vertexCache[num].Position = value - vector2 * 0.025f * 0.5f + vector3 * 0.025f * 0.866f;
				vertexCache[num++].Color = TracerColor;
		...
		{
			_effect = CastleMinerZGame.Instance.Content.Load<Effect>("Shaders\\Tracer");
			...
			_dragonFlashEffect = CastleMinerZGame.Instance.Content.Load<ParticleEffect>("ParticleEffects\\FlashEffect");

...
namespace DNA.CastleMinerZ.AI
{
	public class DragonClientEntity : Entity, IShootableEnemy
	{
		...
		public void TakeExplosiveDamage(Vector3 position, float damageAmount, byte shooterID, InventoryItemIDs itemID)
		{
			...
			float num = IsHeadshot(position) ? 1f : 2.5f;
			damageAmount *= num;
			Health -= damageAmount;
			EnemyManager.Instance.DragonHasBeenHit();
			if (Health <= 0f)
		...
		{
			Bone bone = _dragonModel[0].Skeleton["Bip01 Ponytail1"];
		...
		public override void Update(DNAGame game, GameTime gameTime)
		{
			...
			{
				CurrentInterpolationTime += (float)gameTime.ElapsedGameTime.TotalSeconds * 0.8f;
			}
			else
			{
				CurrentInterpolationTime += (float)gameTime.ElapsedGameTime.TotalSeconds;
			}
			...
			Roll = MathTools.MoveTowardTarget(Roll, Waypoints[0].BaseWpt.TargetRoll, EType.RollRate, (float)gameTime.ElapsedGameTime.TotalSeconds);
			...
			FlapTimer.Update(gameTime.ElapsedGameTime);

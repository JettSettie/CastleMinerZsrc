...
namespace DNA.CastleMinerZ.Terrain.WorldBuilders
{
	internal class CostalBiome : Biome
	{
		...
		private const float worldScale = 3f / 320f;
		...
		public override void BuildColumn(BlockTerrain terrain, int worldX, int worldZ, int minY, float terrainblender)
		{
			...
			{
				num2 += _noiseFunction.ComputeNoise(3f / 320f * (float)worldX * (float)num, 3f / 320f * (float)worldZ * (float)num) / (float)num;
				num *= 2;
			}
			num2 += _noiseFunction.ComputeNoise(0.00234375f * (float)worldX, 0.00234375f * (float)worldZ);
			num2 += _noiseFunction.ComputeNoise(3f / 640f * (float)worldX, 3f / 640f * (float)worldZ) / 2f;
			bool flag = false;

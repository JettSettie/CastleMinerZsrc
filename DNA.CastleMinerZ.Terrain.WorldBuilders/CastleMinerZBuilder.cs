...
namespace DNA.CastleMinerZ.Terrain.WorldBuilders
{
	internal class CastleMinerZBuilder : WorldBuilder
	{
			...
			: base(worldInfo)
		{
			...
			crashSiteDepositer = new CrashSiteDepositer(worldInfo);
			caves.SetLootModifiersByGameMode();
		}
		...
		public override void BuildWorldChunk(BlockTerrain terrain, IntVector3 minLoc)
		{
			...
			for (int i = 0; i < 16; i++)
			{
				...
				for (int j = 0; j < 16; j++)
				{
					...
					{
						crashSiteDepositer.BuildColumn(terrain, num3, num, minLoc.Y, blender2);
					}

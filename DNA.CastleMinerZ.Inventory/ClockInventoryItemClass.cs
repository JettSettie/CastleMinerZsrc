...
namespace DNA.CastleMinerZ.Inventory
{
	public class ClockInventoryItemClass : ModelInventoryItemClass
	{
		...
		public override Entity CreateEntity(ItemUse use, bool attachedToLocalPlayer)
		{
			...
			clockEntity = new ClockEntity(_model, use, attachedToLocalPlayer);
			if (use != 0)
			{
			...
			case ItemUse.UI:
			{
				...
				Matrix localToParent = Matrix.Transform(Matrix.CreateScale(22.4f / clockEntity.GetLocalBoundingSphere().Radius), rotation);
				localToParent.Translation = new Vector3(0f, 0f, 0f);
				clockEntity.LocalToParent = localToParent;
